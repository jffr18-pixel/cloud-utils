...
async function blobToPcm16k(blob) {
  ...
  for (let c = 0; c < chs; c += 1) {
    ...
    for (let i = 0; i < len; i += 1) mono[i] += d[i] / chs;
  }
  const target = 16000;
...
async function sendVoiceNote(blob) {
  ...
  try {
    ...
    alert((converted ? '' : 'No se pudo convertir el audio. ') + err.message);
  }
}
...
async function renderAutomations() {
  ...
  const rev = s.reviews || {};
  $('#auto-rev-enabled').checked = Boolean(rev.enabled);
  $('#auto-rev-url').value = rev.reviewUrl || '';
  $('#auto-rev-text').value = rev.text || '';
  ...
  try {
    await api('automations', {
      ...
      body: {
        ...
        reviews: {
          enabled: $('#auto-rev-enabled').checked,
          reviewUrl: $('#auto-rev-url').value.trim(),
          text: $('#auto-rev-text').value,
        },
        autoCollect: {
